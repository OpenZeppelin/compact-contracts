...
Throws if called by any account whose id hash `H(pk, nonce)` does not match the stored owner commitment.
Use this to only allow the owner to call specific circuits.

Requirements:

- Contract is initialized.
- Caller's id (`H(pk, nonce)`) when used in <<ZOwnablePK-_computeOwnerCommitment,ZOwnablePK-_computeOwnerCommitment>> equals the stored `_ownerCommitment`,
...
The `id` is expected to be computed off-chain as: `id = H(pk, nonce)`
