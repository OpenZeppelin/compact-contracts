...
- *Never used before:* The public key has no prior transaction history on any blockchain network.
- *Never used elsewhere:* The key is exclusively reserved for the specific contract's administrative circuits i.e. `assertOnlyOwner`.
- *Never used again:* The private key is destroyed once ownership is renounced or transferred to another account.
