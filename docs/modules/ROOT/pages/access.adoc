...
```ts
commitment = H(id, instanceSalt, counter, pad(32, "ZOwnablePK:shield:"))
```
...
- Owner identities remain completely private—public keys are never revealed on-chain.
- Ownership transfers are unlinkable—observers cannot correlate past and future ownership.
- Cross-contract attacks are prevented through instance-specific salting.
