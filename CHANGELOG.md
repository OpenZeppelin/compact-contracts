...
- Exclude `crypto/` and `multisig/` from the aggregate `compile` script. They need `--feature-zkir-v3` and are built by `compile:crypto` / `compile:multisig`; recompiling them on v2 emptied their artifact directories. The aggregate cannot move to v3 while `ConfidentialFungibleToken` fails key generation there. `build` moves to `scripts/build.sh`, which runs the builder on v2 without those directories, then compiles them on v3 and copies their sources into `dist`. (#899)
