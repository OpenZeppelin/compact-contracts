...
- **Breaking:** Rename the `ShieldedMultiSigV2` preset module to `NativeShieldedStatelessTreasury` (example `NativeShieldedStatelessTreasuryExample`, mock `MockNativeShieldedStatelessTreasury`), after the modules it composes: `EcdsaSignerManager` and `ShieldedTreasuryStateless`. The threshold assert is now `NativeShieldedStatelessTreasury: threshold cannot exceed 2 (execute verifies at most 2 signatures)`. The replay counter `_nonce` is now exported, so the implementing contract's `ledger()` reader exposes it instead of being empty. Circuit names, parameters, return types and the `execute` digest are unchanged.
- **Breaking:** Rename the `ShieldedMultiSig` preset module to `NativeShieldedTreasury` (example `NativeShieldedTreasuryExample`, mock `MockNativeShieldedTreasury`), after the modules it composes: `Signer`, `ProposalManager` and the stateful `ShieldedTreasury`. Its own asserts now all carry the module name: `NativeShieldedTreasury: recipient must be a shielded user or contract`, `NativeShieldedTreasury: already approved` and `NativeShieldedTreasury: not approved` (the last two were prefixed `Multisig:`). Asserts raised by the composed modules are unchanged, as are circuit names, parameters and return types.
