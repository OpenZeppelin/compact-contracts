...
- **Breaking:** Rename the `ShieldedMultiSigV3` preset module to `NativeShieldedTokenIssuer` (example `NativeShieldedTokenIssuerExample`, mock `MockNativeShieldedTokenIssuer`), and compose `NativeShieldedToken` and `NativeShieldedTokenDerivedNonce` instead of minting and burning inline. The preset's `initialize` drops `initCoinNonce`, gains `name`, `symbol`, `decimals`, and registers the signer set itself (`instanceSalt, tokenDomain, name, symbol, decimals, signerCommitments`), so implementing contracts import only the preset. `mint` returns the minted `ShieldedCoinInfo` and `burn` returns the change coin as `Maybe<ShieldedCoinInfo>` (`none` on a full burn). `getTokenDomain` / `getTokenType` are replaced by `tokenColor`, `name`, `symbol`, `decimals`. Burn rejections now use the token module's messages (`NativeShieldedToken: wrong token`, `NativeShieldedToken: insufficient coin value`), and `mint` rejects a zero recipient (`NativeShieldedToken: invalid recipient`). The example's `ledger()` no longer exposes `_coinNonce` or `_tokenDomain`; coin nonces come from the extension's own counter, independent of the `_counter` replay counter. The mint and burn domain tags are now `NativeShieldedTokenIssuer:mint:` and `NativeShieldedTokenIssuer:burn:`, so every existing signer setup must be re-pointed. (#887)
