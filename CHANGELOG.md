...
- Multisig contract suite under `contracts/src/multisig/`: configurable M-of-N `Signer` / `SignerManager` registry, `ProposalManager`, stateful `ShieldedTreasury` and `ShieldedTreasuryStateless`, `UnshieldedTreasury`, `Forwarder` + `ForwarderPrivate` modules with per-recipient presets, and the `ShieldedProposalMultisig` / `ShieldedStatelessMultisig` presets. Signature verification is stubbed pending ECDSA + Keccak primitives (#475). (#378, #424, #526)
