...
The contract is responsible for three things, and this standard specifies all of them:

- token metadata (`name`, `symbol`, `decimals`, `tokenColor`),
- issuance (`_mint`),
- destruction (`_burn`, `_burnFromContract`).

It also defines two optional extensions: derived-nonce minting, and supply accounting (`totalMinted`, `totalBurned`, and an upper-bound `totalSupply`).
Supply accounting is an extension rather than a core feature because tracking it is a privacy trade-off: a contract-mediated burn is amount-private on its own, and the supply counter is the only thing that would make the burned amount public (see [Supply Accounting](#extension-supply-accounting)).
...
- **Fungible profile** (reference module `NativeShieldedToken`): one token type per contract, the ERC-20-shaped common case.
  ...
  Its optional supply extension uses scalar totals.
- **Family profile** (reference module `NativeShieldedTokenFamily`): many token types per contract, selected by a per-call `domain` parameter.
  ...
  Its optional supply extension uses per-domain maps.
...
Core state is metadata only. The supply counters belong to the optional supply extension (see [Supply Accounting](#extension-supply-accounting)), not the core module.
...
- The optional supply extension adds `_totalMinted` and `_totalBurned` (scalar in the Fungible profile, per-domain maps in the Family profile). See [Supply Accounting](#extension-supply-accounting).
...
Supply accounting is an OPTIONAL extension, not part of the core modules.
The reference implementation ships it as standalone modules (`extensions/NativeShieldedTokenSupply.compact`, scalar; `extensions/NativeShieldedTokenFamilySupply.compact`, per-domain) that a consumer composes alongside the token, pairing the accounting blocks with the matching mint/burn op.
...
When the extension is composed, it tracks the strongest quantities it can and names them for what they are.
...
Composing supply accounting is not free: it makes contract-mediated burn amounts **public**, whereas a bare burn keeps them private.

- A `disclose()` wrapper is a compiler permission marker, not a disclosure.
  Disclosure happens only at a sink: a write to public ledger state, a return from an exported circuit, or a pass to another contract.
- The shielded coin operations (`receiveShielded`, `sendImmediateShielded`, `sendShielded`) emit only commitments and nullifiers (hashes of the coin), never the plaintext value, even though the compiler forces a `disclose()` on each.
- So the **only** thing that puts a burned amount into the public transcript is the supply counter's ledger write (`_totalBurned[domain] = ...`).
  A bare `_burn` / `_burnFromContract` therefore hides the burned amount; composing the extension trades that privacy for an on-chain `totalSupply`.
- Mint amounts are public regardless: `mintShieldedToken` records the amount in the protocol-level `shieldedMints` effect.
  The extension changes burn visibility, not mint visibility.

This is verified against decoded transaction bytes ([study](https://github.com/0xisk/exploring-native-shielded-token-indexing)): an otherwise-identical burn with the supply write removed leaves no amount in the transcript, while every coin-op `disclose()` is still present.

Who can know what:

- **The contract** sees its own mints, and its own burns only when the supply extension is composed.
  Protocol-level activity never calls it, so no contract-side accounting can do better than these counters.
- **An indexer** can always reconstruct mints (`shieldedMints` effects), protocol burns (value deltas), and per-color pool value (negated delta sum).
  It can reconstruct contract-mediated burn amounts only when the supply extension is composed (the counter write is what discloses them); without it, those amounts stay inside commitments.
...
Implementations of the extension MUST maintain the counters as follows.
...
The getters do not gate on token initialization: a counter reading 0 before initialization is correct, and init enforcement stays on the token's mint/burn circuits.
...
3. Contract-initiated outputs carry no coin ciphertext, so wallets cannot currently detect contract-minted coins by scanning the chain.
   The returned coin info is the only copy available to the recipient.
   Callers SHOULD deliver it to the recipient out of band.
4. The caller is responsible for nonce uniqueness.
   Reusing a nonce for the same `(domain, value, recipient)` produces a duplicate commitment, which the ledger rejects.
5. With a secret, cryptographically random nonce, the commitment cannot be linked to a recipient.
   This is the recipient-private mint.
   For operator-driven flows, the commitment can be computed off-chain before submission.

A consumer composing the supply extension MUST pair every successful mint with `_addMinted(amount)` under the same `domain`, reverting on overflow (see [Supply Accounting](#extension-supply-accounting)).
...
3. MUST send `amount` to `shieldedBurnAddress()`.
   The bare burn keeps the burned amount private. A consumer composing the supply extension MUST also add `amount` to `_totalBurned[domain]`, which makes the amount public (see [Supply Accounting](#extension-supply-accounting)).
...
First, when supply is tracked at all, it is tracked as bounds, not as exact supply.
An exact-looking `totalSupply` counter is strictly worse: it reports the same number while implying a guarantee the protocol cannot provide, because out-of-band burns are invisible to contract state.
Naming the quantities `totalMinted`, `totalBurned`, and an upper-bound `totalSupply` gives indexers correct semantics.

Second, supply tracking is an optional extension rather than a core feature, because composing it costs burn-amount privacy.
A contract-mediated burn is amount-private on its own: the coin primitives disclose only commitments and nullifiers, and the `disclose()` they require is a permission marker, not a sink.
The supply counter's ledger write is the single thing that would put the burned amount into the public transcript (see [Supply Accounting](#extension-supply-accounting)).
An issuer that values burn privacy omits the extension; one that values on-chain auditability composes it.
Folding the counters into the core would force that disclosure on everyone.

The cost of making it opt-in is that the choice is permanent: Compact ledger layouts are fixed at deployment, so a token deployed without the supply extension can never add it later, and one deployed with it can never reclaim the burn privacy.
Issuers MUST decide at deployment.
Mint amounts are unaffected either way: they are public at the protocol level via the `shieldedMints` effect, so `_addMinted` adds accounting, not disclosure.
...
Composing the supply extension makes contract-mediated burn amounts public; a token that omits it keeps them private (see [Supply Accounting](#extension-supply-accounting)).
Issuers MUST make this choice at deployment, since the ledger layout is then fixed.
...
1. **New Compact modules.** [`NativeShieldedToken.compact` (Fungible profile) and `NativeShieldedTokenFamily.compact` (Family profile)](https://github.com/OpenZeppelin/compact-contracts/tree/main/contracts/src/token) in the OpenZeppelin Compact Contracts library: the core state and circuits specified above, composed with the library's `Utils` module (initialization is tracked inline per-module, not via the shared `Initializable` module), plus the optional extension modules `extensions/NativeShieldedTokenDerivedNonce.compact`, `extensions/NativeShieldedTokenSupply.compact`, and `extensions/NativeShieldedTokenFamilySupply.compact`.
2. **Mocks, simulators, and tests.** `MockNativeShieldedToken.compact` and `MockNativeShieldedTokenFamily.compact` composing the modules (including the supply extension) and exposing their circuits, with TypeScript simulators and Vitest suites.
...
- Supply extension: `totalSupply == totalMinted - totalBurned` after arbitrary mint/burn sequences; unknown domains read 0; getters do not gate on token initialization (a counter reading 0 pre-init is correct).
