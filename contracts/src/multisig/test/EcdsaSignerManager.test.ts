...
describe('EcdsaSignerManager', () => {
  beforeEach(() => {
    verifier = new EcdsaSignerManagerSimulator(SALT, SIGNERS, THRESHOLD);
  });

  describe('initialization', () => {
    it('should register all signer commitments', () => {
      expect(verifier.getSignerCount()).toEqual(3n);
      expect(verifier.getThreshold()).toEqual(2n);
      expect(verifier.isSigner(COMMITMENT1)).toEqual(true);
      expect(verifier.isSigner(COMMITMENT2)).toEqual(true);
      expect(verifier.isSigner(COMMITMENT3)).toEqual(true);
    });

    it('should not recognize an unregistered commitment', () => {
      const unknown = EcdsaSignerManagerSimulator.calculateSignerId(
        PK_UNKNOWN,
        SALT,
      );
      expect(verifier.isSigner(unknown)).toEqual(false);
    });
  });
  ...
  describe('verify', () => {
    it('should pass with two distinct registered signers', () => {
      expect(() => verifier.verify(MSG, [PK1, PK2], [SIG, SIG])).not.toThrow();
    });

    it('should reject a duplicate signer', () => {
      expect(() => verifier.verify(MSG, [PK1, PK1], [SIG, SIG])).toThrow(
        'EcdsaSignerManager: duplicate signer',
      );
    });

    it('should reject an unregistered signer', () => {
      expect(() => verifier.verify(MSG, [PK_UNKNOWN, PK2], [SIG, SIG])).toThrow(
