...
// ---------------------------------------------------------------------------
// Identity and hashing (pure circuits)
// ---------------------------------------------------------------------------

describe('ConfidentialNoteFungibleToken: identity and hashing', () => {
  beforeEach(async () => {
...
describe('ConfidentialNoteFungibleToken: _mint and _mintNote', () => {
  beforeEach(async () => {
    token = await ConfidentialNoteFungibleTokenSimulator.create();
  });
...
describe('ConfidentialNoteFungibleToken: transfer', () => {
  ...
  it('should not let anyone other than the owner spend', async () => {
    ...
    await expect(token.transfer(CAROL, 30n)).rejects.toThrow(
      'wit_Path: commitment not found in tree',
    );
...
// ---------------------------------------------------------------------------
// _transfer / _burn: the conserving building blocks
// ---------------------------------------------------------------------------

describe('ConfidentialNoteFungibleToken: _transfer and _burn', () => {
  let input: Note;
...
describe('ConfidentialNoteFungibleToken: _consumeNote', () => {
  ...
  it('should not consume a note under an owner pk it was not committed to', async () => {
    await expect(token._consumeNote(BOB)).rejects.toThrow(
      'wit_Path: commitment not found in tree',
    );
  });
...
describe('ConfidentialNoteFungibleToken: nonce hygiene', () => {
  beforeEach(async () => {
    token = await ConfidentialNoteFungibleTokenSimulator.create();
  });
