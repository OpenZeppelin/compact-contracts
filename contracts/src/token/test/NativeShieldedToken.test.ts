...
const deploy = (init = INIT): NativeShieldedTokenSimulator =>
  new NativeShieldedTokenSimulator(DOMAIN, SEED, NAME, SYMBOL, DECIMALS, init);
...
describe('NativeShieldedToken (Fungible profile)', () => {
  describe('initialization', () => {
    ...
    it('should expose the constructor metadata (INV-14)', () => {
      expect(token.name()).toEqual(NAME);
      expect(token.symbol()).toEqual(SYMBOL);
      expect(token.decimals()).toEqual(DECIMALS);
    });

    it('should report _isInitialized true after construction (INV-15)', () => {
      expect(token.isInitialized()).toBe(true);
    });

    it('should compute tokenColor as a 32-byte value at call time (INV-1)', () => {
      ...
      expect(token.tokenColor()).toEqual(color);
    });

    it('should start with zero supply counters (INV-2, INV-4)', () => {
      expect(token.totalMinted()).toBe(0n);
      expect(token.totalBurned()).toBe(0n);
  ...
  describe('before initialization', () => {
    ...
    it.each(circuitsToFail)(
      ...
      (method, args) => {
        ...
        }).toThrow('NativeShieldedToken: contract not initialized');
      },
    );

    it('should report zero supply before initialize (supply getters do not gate on init) (INV-2, INV-4)', () => {
    ...
    it('should revert _deriveNonce before the chain is seeded (INV-13)', () => {
      expect(() => token._deriveNonce()).toThrow(
  ...
  describe('_mint', () => {
    ...
    it('should return a coin with color = tokenColor, value = amount, nonce = arg (INV-1)', () => {
      const nonce = b32('mint-nonce-1');
      const coin = token._mint(RECIPIENT, AMOUNT, nonce);
      expect(coin.value).toBe(AMOUNT);
      expect(coin.nonce).toEqual(nonce);
      expect(coin.color).toEqual(token.tokenColor());
    });

    it('should mint to a contract-address recipient (INV-1)', () => {
      const coin = token._mint(RECIPIENT_CONTRACT, AMOUNT, b32('mint-c'));
      expect(coin.value).toBe(AMOUNT);
      expect(coin.color).toEqual(token.tokenColor());
    });

    it('should increment totalMinted by amount (INV-2)', () => {
      token._mint(RECIPIENT, AMOUNT, b32('m1'));
      expect(token.totalMinted()).toBe(AMOUNT);
      token._mint(RECIPIENT, 500n, b32('m2'));
      expect(token.totalMinted()).toBe(AMOUNT + 500n);
    });

    it('should revert on a zero recipient key (INV-6)', () => {
      expect(() => token._mint(ZERO_KEY, AMOUNT, b32('z'))).toThrow(
        'NativeShieldedToken: invalid recipient',
      );
    });

    it('should revert on a zero recipient address (INV-6)', () => {
      expect(() => token._mint(ZERO_ADDRESS, AMOUNT, b32('z'))).toThrow(
        'NativeShieldedToken: invalid recipient',
      );
    });
  ...
  describe('_burn (same-tx coin)', () => {
    ...
    it('should revert on a wrong-color coin (INV-1)', () => {
      expect(() => token._burn(coinOf(AMOUNT, b32('wrong')), AMOUNT, REFUND_TO)).toThrow(
        'NativeShieldedToken: wrong token',
      );
    });

    it('should revert when amount > coin.value (INV-8)', () => {
      expect(() => token._burn(coinOf(AMOUNT), AMOUNT + 1n, REFUND_TO)).toThrow(
        'NativeShieldedToken: insufficient coin value',
      );
    });

    it('should revert on a zero refundTo (INV-7)', () => {
      expect(() => token._burn(coinOf(AMOUNT), 1n, ZERO_KEY)).toThrow(
        'NativeShieldedToken: invalid refund target',
      );
      expect(() => token._burn(coinOf(AMOUNT), 1n, ZERO_ADDRESS)).toThrow(
        'NativeShieldedToken: invalid refund target',
      );
    });

    it('should return none on a full burn (amount == coin.value) (INV-10)', () => {
      const res = token._burn(coinOf(AMOUNT), AMOUNT, REFUND_TO);
      expect(res.is_some).toBe(false);
    });

    it('should return some(refund) with refund.value == coin.value - amount on a partial burn (INV-10)', () => {
      const res = token._burn(coinOf(AMOUNT), 600n, REFUND_TO);
      expect(res.is_some).toBe(true);
      expect(res.value.value).toBe(AMOUNT - 600n);
    });

    it('should increment totalBurned by amount (INV-4)', () => {
      token._burn(coinOf(AMOUNT), AMOUNT, REFUND_TO);
      expect(token.totalBurned()).toBe(AMOUNT);
    });
  });
  ...
  describe('_burnFromContract (contract-held coin)', () => {
    ...
    it('should revert on a wrong-color coin (INV-1)', () => {
      expect(() => token._burnFromContract(qCoinOf(AMOUNT, b32('wrong')), AMOUNT)).toThrow(
        'NativeShieldedToken: wrong token',
      );
    });

    it('should revert when amount > coin.value (INV-8)', () => {
      expect(() => token._burnFromContract(qCoinOf(AMOUNT), AMOUNT + 1n)).toThrow(
        'NativeShieldedToken: insufficient coin value',
      );
    });

    it('should return change and increment totalBurned on a partial burn (INV-4, INV-10)', () => {
      const res = token._burnFromContract(qCoinOf(AMOUNT), 600n);
      expect(res.is_some).toBe(true);
      expect(token.totalBurned()).toBe(600n);
    });

    it('should return none and increment totalBurned on a full burn (INV-4, INV-10)', () => {
      const res = token._burnFromContract(qCoinOf(AMOUNT), AMOUNT);
      expect(res.is_some).toBe(false);
      expect(token.totalBurned()).toBe(AMOUNT);
    });
  });
  ...
  describe('supply accounting', () => {
    ...
    it('should report totalSupply == totalMinted - totalBurned after a mint/burn sequence (INV-5)', () => {
      const color = token.tokenColor();
      token._mint(RECIPIENT, AMOUNT, b32('s1'));
      token._mint(RECIPIENT, 500n, b32('s2'));
      token._burn({ nonce: b32('c'), color, value: 400n }, 400n, REFUND_TO);
      expect(token.totalMinted()).toBe(1_500n);
      expect(token.totalBurned()).toBe(400n);
      expect(token.totalSupply()).toBe(1_100n);
    });
    ...
    it('should be drivable into burned > minted under --skip-zk (MED-1 boundary)', () => {
      const color = token.tokenColor();
      // No mint; burn a fabricated coin of the right color.
      token._burn({ nonce: b32('c'), color, value: AMOUNT }, AMOUNT, REFUND_TO);
      expect(token.totalMinted()).toBe(0n);
      expect(token.totalBurned()).toBe(AMOUNT);
