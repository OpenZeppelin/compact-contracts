...
// Live-backend run of the integration specs against the local stack
// (`make env-up`). Same spec files as the dry `test:integration`; only the
// backend (via `MIDNIGHT_BACKEND=live`) and this config differ — each
// `await Sim.create()` deploys + attaches a real composed contract through the
// registered live harness.
//
