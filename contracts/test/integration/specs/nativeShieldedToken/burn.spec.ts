 ...
 * return-shape and accounting in the simulator.
 *
 * Verifies on the proof loop: wrong-color rejection (the only barrier, since
 * the protocol receive does not validate color), zero refundTo, and
 * amount > coin.value.
 */
describe('Burn — revert guards (proof loop)', () => {
  ...
  it('should reject burning a wrong-color coin', async () => {
    const coin = { nonce: b32('c'), color: b32('wrong-color'), value: 1_000n };
    await expect(
      kit.deployed.callTx._burn(coin, 1_000n, refundTo),
    ).rejects.toThrow('NativeShieldedToken: wrong token');
  });

  it('should reject when amount > coin.value', async () => {
    const coin = { nonce: b32('c'), color, value: 1_000n };
    await expect(
      kit.deployed.callTx._burn(coin, 1_001n, refundTo),
    ).rejects.toThrow('NativeShieldedToken: insufficient coin value');
  });

  it('should reject a zero refundTo', async () => {
    const coin = { nonce: b32('c'), color, value: 1_000n };
    await expect(
      kit.deployed.callTx._burn(coin, 500n, ZERO_REFUND),
    ).rejects.toThrow('NativeShieldedToken: invalid refund target');
  });
});

/**
 * `_burnFromSelf` revert guards (proof loop). Same reasoning: color and
 ...
 *
 * Verifies wrong-color rejection and amount > coin.value.
 */
describe('BurnFromSelf — revert guards (proof loop)', () => {
  let kit: NativeShieldedTokenV1Kit;
  let color: Uint8Array;
  ...
  it('should reject a wrong-color contract-held coin', async () => {
    const coin = {
      nonce: b32('q'),
      color: b32('wrong-color'),
      value: 1_000n,
      mt_index: 0n,
    };
    await expect(
      kit.deployed.callTx._burnFromSelf(coin, 1_000n),
    ).rejects.toThrow('NativeShieldedToken: wrong token');
  });
  ...
  it('should reject when amount > coin.value', async () => {
    ...
    await expect(
      kit.deployed.callTx._burnFromSelf(coin, 1_001n),
    ).rejects.toThrow('NativeShieldedToken: insufficient coin value');
  });
});
