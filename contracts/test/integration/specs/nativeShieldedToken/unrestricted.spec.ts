 ...
 *
 * Authorization gating (INV-16) is out of scope for this suite (presets
 * removed; consumer concern).
 */
describe('Unrestricted — any caller can mint (no auth gate)', () => {
  ...
  it('should let a non-deployer (ALICE) mint to herself with no role grant', async () => {
    ...
    const res = await handle.callTx._mint(alice, 1_000n, b32('alice-unrestricted'));
