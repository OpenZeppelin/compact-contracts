...
describe('Privacy — recipient (un)linkability (HIGH-1, P0)', () => {
  ...
  it('should NOT expose a secret-nonce mint recipient in public data (recipient-private)', async () => {
    ...
    const res = await kit.deployed.callTx._mint(selfRecipient, 1_000n, secretNonce);

    const blob = publicEffectsHexBlob(res.public);
    // The secret nonce is the linchpin: it is not present in any public field,
    // so the on-chain commitment cannot be recomputed by an enumerator.
  ...
  it('should make a derived-nonce mint recipient recoverable by key enumeration (recipient-public)', async () => {
    const res = await kit.deployed.callTx._mintWithDerivedNonce(selfRecipient, 2_000n);
    ...
    const realCommitment = coinCommitment(ledgerCoin, recipientCpk).toLowerCase();
    expect(blob.includes(realCommitment)).toBe(true);

    // ... a wrong candidate key does not.
    const wrongCommitment = coinCommitment(ledgerCoin, sampleCoinPublicKey()).toLowerCase();
