...
describe('TestToken — functional re-verification after VK rotation', () => {
  ...
  it('rotate `_mint` VK; mint succeeds; balance increments', async () => {
    const before =
  ...
  it('rotate `pause` VK; pause(); paused state is true', async () => {
  ...
  it('rotate `grantRole` VK; admin grants MINTER to ALICE; observed', async () => {
  ...
  it('rotate `transfer` VK; transfer ALICE → BOB; both balances move', async () => {
