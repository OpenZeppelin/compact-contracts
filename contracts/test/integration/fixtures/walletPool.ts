 ...
 * `MidnightWalletProvider.start()` performs a full sync against the local
 ...
 * `deployTestTokenV1` calls reuse the already-warm wallets.
 *
 * The contract is redeployed fresh per spec (each `deployTestTokenV1` returns
 ...
 * `deployTestTokenV1` and the kit's `teardown()` will own that pool's
 * lifecycle.
 */
...
let sharedEnv: EnvironmentConfiguration | undefined;

export function getSharedWalletPool(
  ...
  assertSameEnv(sharedEnv as EnvironmentConfiguration, env);
  return shared;
}

export function getSharedSigners(env: EnvironmentConfiguration): Signers {
...
function assertSameEnv(
  a: EnvironmentConfiguration,
  b: EnvironmentConfiguration,
): void {
