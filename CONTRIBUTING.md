...
The run has three layers. A plan job asks the runner which live targets exist (the same list `yarn test:live --list` prints) and which spec files each one holds. A compile job per target then builds that target's contract slice once with real ZK keys and uploads it, and a suite job per **spec file** downloads its target's build and runs that one file on its own runner, stack, and 6-hour job budget.

Per file rather than per target because a target is one long serial job, while the files are independent: every suite job resets its own stack, so nothing one spec leaves on the node can fail another, and a failure names the file that caused it. The compile layer is what makes that affordable, since 16 token spec jobs must not mean 16 token compiles. Because `needs` is job-level in Actions, the suite matrix waits for the whole compile matrix rather than for its own target's job; that barrier is minutes against a suite measured in hours.

Every job runs the same `yarn test:live` entry point a local run does, so the two-round flake semantics are identical. The two flags CI adds are `--compile-only` (build the slice and stop) and `--prebuilt` (run against a downloaded build, and abort rather than compile if it is incomplete or carries a truncated key). A suite job whose compile job failed fails at the download step rather than being skipped, so one broken target never costs the other targets their night. Each job's verdict lands in its GitHub job summary, the JSON verdict reports upload as a `live-reports-<target>-<file>` artifact on every run, and the service and worker logs as `live-logs-<target>-<file>` on failure.
